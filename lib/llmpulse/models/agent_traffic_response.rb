...
module LLMPulse
  class AgentTrafficResponse < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'timeseries' => :'Hash<String, Hash<String, Integer>>',
        :'request_id' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LLMPulse::AgentTrafficResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LLMPulse::AgentTrafficResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'timeseries')
        if (value = attributes[:'timeseries']).is_a?(Hash)
    ...
    def valid?
      ...
      group_by_validator = EnumAttributeValidator.new('String', ["bot", "company"])
      return false unless group_by_validator.valid?(@group_by)
      granularity_validator = EnumAttributeValidator.new('String', ["day", "week", "month"])
    ...
    def group_by=(group_by)
      validator = EnumAttributeValidator.new('String', ["bot", "company"])
    ...
    def hash
      [project_id, from, to, group_by, granularity, totals, timeseries, request_id].hash
    end
